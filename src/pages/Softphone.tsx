...
export default function Softphone() {
  ...
  type VoiceEventHandler = (...args: unknown[]) => void;
  const deviceRef = useRef<{ disconnectAll?: () => void; audio?: { incoming?: () => Promise<unknown> }; on?: (event: string, handler: VoiceEventHandler) => void } | null>(null);
  const callRef = useRef<{ disconnect?: () => void; mute?: (v: boolean) => void; on?: (event: string, handler: VoiceEventHandler) => void } | null>(null);
  ...
  const answer = () => {
    const call = callRef.current as { accept?: () => void; on?: (event: string, handler: VoiceEventHandler) => void } | null;
    call?.accept?.();
    setCallState("open");
    call?.on?.("disconnect", () => { setCallState("idle"); callRef.current = null; });
  };
