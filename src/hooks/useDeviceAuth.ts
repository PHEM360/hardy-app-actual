...
export function useDeviceAuth() {
  ...
  // Survives the resolve() re-run that signOut() itself triggers (user
  // becomes null), so a revoked device settles on "revoked" instead of
  // flashing straight back to a fresh QR code as if nothing had happened.
  ...
  const restartPairing = useCallback(() => {
    // Needed from the "revoked" screen too: nothing else would otherwise
    // move status away from "revoked" to let a fresh pairing attempt start.
    revokedRef.current = false;
